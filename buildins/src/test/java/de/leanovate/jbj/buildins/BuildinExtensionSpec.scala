...
class BuildinExtensionSpec extends SpecificationWithJUnit {
  "BuilinsExtensions" should {
    "Contain function list" in {
      val functionNames = BuildinsExtension.functions.map(_.name.toString.toLowerCase).toSet
      ...
      functionNames must containAllOf(Seq("zend_version"))
