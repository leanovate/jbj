...
class ForeachObjectsSpec extends SpecificationWithJUnit with TestJbjExecutor {
  "foreach with objects" should {
    ...
    "Foreach loop tests - modifying the object during the loop." in {
      // lang/foreachLoopObjects.003.phpt
      script(
        """<?php
          |
          |class C {
    ...
    "Foreach loop tests - Removing the current element from an iterated object." in {
      // lang/foreachLoopObjects.004.phpt
      script(
        """<?php
          |
          |class C {
        ...
        """
          ...
          |string(10) "Original b"
          |
          |Notice: Undefined property: C::$b in /lang/ForeachObjectsSpec.inlinePhp on line 15
          |string(10) "Original c"
          |
          |Notice: Undefined property: C::$b in /lang/ForeachObjectsSpec.inlinePhp on line 15
          |string(10) "Original d"
    ...
    "Foreach loop tests - removing properties before and after the current property during the loop." in {
      // lang/foreachLoopObjects.005.phpt
      script(
        """<?php
          |
          |class C {
    ...
    "Foreach loop tests - substituting the entire iterated entity during the loop." in {
      // lang/foreachLoopObjects.006.phpt
      script(
        """<?php
          |
          |class C {
