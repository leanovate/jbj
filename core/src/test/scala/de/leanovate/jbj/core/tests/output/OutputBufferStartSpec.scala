...
class OutputBufferStartSpec extends SpecificationWithJUnit with TestJbjExecutor {
  "ob_start()" should {
    "Test return type and value for ob_start()" in {
      // output/ob_start_basic_001.phpt
      script(
        """<?php
          |/*
    ...
    "ob_start(): ensure even fatal error test is affected by output buffering." in {
      // output/ob_start_basic_003.phpt
      script(
        """<?php
          |
          |function f() {
        ...
        """I have stolen your output""".stripMargin
      )
    }

    "ob_start() chunk_size: confirm buffer is flushed after any output call that causes its length to equal or exceed chunk_size." in {
      // output/ob_start_basic_004.phpt
      script(
        """<?php
          |/*
          | * proto bool ob_start([ string|array user_function [, int chunk_size [, bool erase]]])
